...
import (
	...
	"github.com/benfiola/homelab-images/gateway-route-sync/internal/reconciler"
	"github.com/benfiola/homelab-images/gateway-route-sync/internal/scheme"
...
func New(opts *Opts) (*Controller, error) {
	...
	manager, err := controllerruntime.NewManager(kubeConfig, controllerruntime.Options{
		...
		LeaderElectionID:       "gateway-route-sync.homelab-images.benfiola.com",
	...
	reconcilers := []reconciler.Reconciler{
		&reconciler.WrappedGatewayReconciler{Client: client, Scheme: builtScheme},
		&reconciler.RouteReconciler{Client: client, Scheme: builtScheme},
	}
