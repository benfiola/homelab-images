...
def get_all_components():
    ...
    for d in repo_root.iterdir():
        if d.is_dir() and not d.name.startswith('.') and d.name not in excluded:
            # Check if it's a valid component (has .goreleaser.yaml)
            if (d / '.goreleaser.yaml').exists():
                components.append(d.name)
    return sorted(components)

def get_changed_components():
    """Detect changed components based on dirty files or git diff."""
    valid_components = set(get_all_components())

    # First, check for dirty files (uncommitted changes)
    dirty_files = run('git diff --name-only')
    if dirty_files:
        changed_files = dirty_files.split('\n')
    else:
        # No dirty files, fall back to comparing refs
        ...
        changed_files_str = run(cmd)
        changed_files = changed_files_str.split('\n')

    # Extract component names (first directory in path) and filter against valid components
    components = set()
