...
This CLI exposes a few subcommands that each have unique configuration requirements.
...
| ----------------------------- | --------------------- | --------------------- | ------------------------------------------------------------------------------------------------- |
| `AC_GAME_DATA_URL`            | `--game-data-url`     | —                     | URL of the game data archive to download and extract to `--data-dir`. **Required.**               |
| `AC_LOGIN_DATABASE_INFO`      | `--login-db`          | —                     | Auth database connection string (`host;port;user;pass;dbname`). **Required.**                     |
| `AC_WORLD_DATABASE_INFO`      | `--world-db`          | —                     | World database connection string (`host;port;user;pass;dbname`). **Required.**                    |
| `AC_CHARACTER_DATABASE_INFO`  | `--character-db`      | —                     | Character database connection string (`host;port;user;pass;dbname`). **Required.**                |
| `AC_PLAYERBOTS_DATABASE_INFO` | `--playerbots-db`     | —                     | Playerbots database connection string (`host;port;user;pass;dbname`). **Required.**               |
| `AC_DATA_DIR`                 | `--data-dir`          | `/data`               | Directory where game data is stored.                                                              |
...
| ------------------------ | ------------ | ------------------ | ----------------------------------------------------------------------------- |
| `AC_LOGIN_DATABASE_INFO` | `--login-db` | —                  | Auth database connection string (`host;port;user;pass;dbname`). **Required.** |
| `AC_DATA_DIR`            | `--data-dir` | `/data`            | Path to the game data directory.                                              |
| `AC_LOGS_DIR`            | `--logs-dir` | `/logs`            | Directory where log files are written.                                        |
| `AC_TEMP_DIR`            | `--temp-dir` | `/tmp/azerothcore` | Directory used for temporary files.                                           |
