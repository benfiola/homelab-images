...
import (
	...
	"github.com/benfiola/homelab-images/bucket-sync/internal/reconciler"
	"github.com/benfiola/homelab-images/bucket-sync/internal/scheme"
...
func New(opts *Opts) (*Controller, error) {
	...
	manager, err := controllerruntime.NewManager(kubeConfig, controllerruntime.Options{
		...
		LeaderElectionID:       "bucket-sync.homelab-images.benfiola.com",
	...
	reconcilers := []reconciler.Reconciler{
		&reconciler.BucketSyncPolicyReconciler{Client: client, Scheme: builtScheme},
		&reconciler.BucketSyncReconciler{Namespace: opts.Namespace, Client: client, Scheme: builtScheme},
